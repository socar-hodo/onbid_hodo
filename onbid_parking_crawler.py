...
try:
    ...
    while True:
        ...
        for row in rows:
            ...
            # fn_selectDetail 숫자 추출
            nums = re.findall(r"'([^']+)'", href)
            if len(nums) != 6:
                continue

            # 공고번호는 title 속성에서 추출 (깨끗함)
            ...
            # 물건상태
            status_match = re.search(r"(인터넷입찰진행중|일반경쟁|제한경쟁)", full_text)
            status = status_match.group() if status_match else "-"

            # 🔥 진짜 상세페이지 URL 생성
            detail_url = (
                "https://www.onbid.co.kr/op/cta/cltrdtl/"
                "collateralDetailRealEstateDetail.do?"
                f"cltrNo={nums[0]}"
                f"&plnmNo={nums[1]}"
                f"&pbctNo={nums[2]}"
                f"&scrnNo={nums[3]}"
        ...
        for idx, item in enumerate(all_parking_data[:20], 1):
            ...
            blocks = {
                "blocks": [
                    ...
                    {
                        "type": "section",
                        "text": {"type": "mrkdwn", "text": f"🔗 <{item['link']}|공고 상세보기>"}
                    },
                    {"type": "divider"}
                ]
            }
